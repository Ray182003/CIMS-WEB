...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] === 'delete') {
        $archiveSql = "UPDATE death_certificates SET is_archived = 1, archived_at = NOW() WHERE id = ?";
        $archiveStmt = $conn->prepare($archiveSql);
        if ($archiveStmt->execute([$id])) {
            echo "<script>alert('Death certificate archived successfully!'); window.location.href='index.php';</script>";
            exit;
        } else {
    ...
    if ($_GET['action'] === 'restore') {
        $restoreSql = "UPDATE death_certificates SET is_archived = 0, archived_at = NULL WHERE id = ?";
        $restoreStmt = $conn->prepare($restoreSql);
        if ($restoreStmt->execute([$id])) {
            echo "<script>alert('Death certificate restored successfully!'); window.location.href='index.php';</script>";
            exit;
        } else {
    ...
    if ($_GET['action'] === 'permanent_delete') {
        $deleteSql = "DELETE FROM death_certificates WHERE id = ?";
        $deleteStmt = $conn->prepare($deleteSql);
        if ($deleteStmt->execute([$id])) {
            echo "<script>alert('Death certificate permanently deleted!'); window.location.href='../archive/index.php';</script>";
            exit;
        } else {
...
<style>
    ...
    .death-year-dropdown .year-dropdown-header {
        display: flex;
        align-items: center;
        gap: 0.5rem;
    ...
    .death-year-dropdown .year-range {
        flex: 1;
        text-align: center;
        font-weight: 600;
        font-size: 0.95rem;
    ...
    .death-year-dropdown .year-nav:hover,
    .death-year-dropdown .year-nav:focus-visible {
        background: rgba(13, 110, 253, 0.2);
        color: #0a58ca;
        outline: none;
    }

    .death-year-dropdown .year-clear {
        border: none;
        background: none;
        color: #0d6efd;
        font-size: 0.8rem;
    ...
    .death-year-dropdown .year-grid {
        display: grid;
        grid-template-columns: repeat(4, minmax(0, 1fr));
        gap: 0.45rem;
    }

    .death-year-dropdown .year-cell {
        border: none;
        border-radius: 10px;
        padding: 0.55rem 0;
        background: #f4f7fb;
    ...
    .death-year-dropdown .year-cell:hover,
    .death-year-dropdown .year-cell:focus-visible {
        background: rgba(13, 110, 253, 0.14);
        color: #0d6efd;
        outline: none;
    }

    .death-year-dropdown .year-cell.active {
        background: #0d6efd;
        color: #ffffff;
    ...
    .death-panel{border-radius:1.15rem;box-shadow:0 22px 48px rgba(15,40,77,.18);border:1px solid rgba(15,40,77,.12)}
    .death-panel .card-header{border-radius:1.15rem 1.15rem 0 0;padding:1rem 1.35rem;background:linear-gradient(135deg,#0d6efd,#6610f2);border-bottom:none;display:flex;align-items:center;justify-content:space-between}
    ...
    .death-panel .page-item .page-link{border:none;border-radius:.75rem;padding:.45rem .85rem;font-weight:600;color:#0d366b;background:rgba(255,255,255,.9);box-shadow:0 10px 18px rgba(15,40,77,.12);transition:transform .18s ease, box-shadow .18s ease, background .18s ease}
    .death-panel .page-item .page-link:hover,.death-panel .page-item .page-link:focus-visible{transform:translateY(-2px);background:rgba(13,110,253,.18);color:#0b2c61}
    .death-panel .page-item.active .page-link{background:linear-gradient(135deg,#0d6efd,#6610f2);color:#fff;box-shadow:0 14px 24px rgba(13,110,253,.28)}
    .death-panel .page-item.disabled .page-link{opacity:.45;transform:none;box-shadow:none}
    .death-panel .pagination{justify-content:flex-end}

    .death-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:1rem;margin-bottom:1.25rem}
    .summary-card{display:flex;gap:.75rem;align-items:flex-start;background:#fff;border:1px solid rgba(15,40,77,.12);box-shadow:0 16px 36px rgba(15,40,77,.16);border-radius:1rem;padding:1rem}
    .summary-card.primary{background:linear-gradient(180deg, rgba(13,110,253,.08), rgba(102,16,242,.08)), #fff}
    .summary-icon{font-size:1.4rem;width:2.2rem;height:2.2rem;border-radius:.75rem;background:#f3f6fb;display:inline-flex;align-items:center;justify-content:center;color:#0d366b}
    .summary-label{font-weight:700;color:#0f284d;font-size:.85rem;text-transform:uppercase;letter-spacing:.06em}
    .summary-value{font-weight:800;font-size:1.25rem;color:#0f284d}
    .summary-meta{font-size:.8rem;color:#5b6e87}
</style>
                ...
                <div class="container-fluid px-4 pt-3">
                    <?php if (isset($_GET['restore_success']) && $_GET['restore_success'] == 1): ?>
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            ...
                            $tables = $_GET['tables'] ?? '';
                            $count = $_GET['count'] ?? 0;
                            echo "Restored $count records to Death Records!";
                            ?>
                    ...
                    <div class="row mb-3">
                        <div class="col-lg-4 mb-lg-0 mb-3 col-12">
                            <input class="form-control" type="search" placeholder="Search" id="search-input">
                        </div>
                        <div class="col-md-3">
                            <input type="text" id="yearpicker" class="form-control" placeholder="Search year">
                        </div>
                        <div class="col-lg-auto d-grid ms-auto">
                            <a class="btn btn-primary rounded-pill px-4" href="insert.php">New</a>
                        </div>
                    </div>

                    <div class="row gap-3">
                        <div class="col-12">
                            <div class="card border-0 death-panel">
                                <div class="card-header text-white">
                                    <div class="d-flex align-items-center gap-3">
                                        <span class="header-icon" aria-hidden="true">🕯️</span>
                                        <div>
                                            <h5 class="card-title mb-0">Death Certificate List</h5>
                                            <span class="card-subtitle">Central overview of death records</span>
                                        </div>
                                    </div>
