...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    if ($_GET['action'] === 'delete') {
        $archiveSql = "UPDATE liberty_records SET is_archived = 1, archived_at = NOW() WHERE id = ?";
        $stmt = $conn->prepare($archiveSql);
        if ($stmt->execute([$id])) {
            echo "<script>alert('Liberty record archived successfully!'); window.location.href='index.php';</script>";
            exit;
        } else {
    ...
    if ($_GET['action'] === 'restore') {
        $restoreSql = "UPDATE liberty_records SET is_archived = 0, archived_at = NULL WHERE id = ?";
        $stmt = $conn->prepare($restoreSql);
        if ($stmt->execute([$id])) {
            echo "<script>alert('Liberty record restored successfully!'); window.location.href='index.php';</script>";
            exit;
        } else {
    ...
    if ($_GET['action'] === 'permanent_delete') {
        $deleteSql = "DELETE FROM liberty_records WHERE id = ?";
        $deleteStmt = $conn->prepare($deleteSql);
        if ($deleteStmt->execute([$id])) {
            echo "<script>alert('Liberty record permanently deleted!'); window.location.href='../archive/index.php';</script>";
            exit;
        } else {
...
<style>
    ...
    .liberty-controls{display:flex;gap:.75rem;align-items:center;margin-bottom:1.1rem}
    .liberty-controls .controls-left{display:flex;gap:.75rem;flex-wrap:wrap;align-items:center}
    .liberty-controls .controls-right{margin-left:auto;display:flex;align-items:center}
    .liberty-controls .form-control{border-radius:6px;padding:.5rem .9rem;border:1px solid #d6d9e0;background:#fff;box-shadow:0 6px 14px rgba(15,96,180,.08)}
    .liberty-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-bottom:1.5rem}
    ...
    .liberty-panel{border-radius:1.15rem;box-shadow:0 22px 48px rgba(15,40,77,.18);border:1px solid rgba(15,40,77,.12)}
    .liberty-panel .card-header{border-radius:1.15rem 1.15rem 0 0;padding:1rem 1.35rem;background:linear-gradient(135deg,#0d6efd,#6610f2);border-bottom:none;display:flex;align-items:center;justify-content:space-between}
    ...
    .liberty-panel .page-item .page-link{border:none;border-radius:.75rem;padding:.45rem .85rem;font-weight:600;color:#0d366b;background:rgba(255,255,255,.9);box-shadow:0 10px 18px rgba(15,40,77,.12);transition:transform .18s ease, box-shadow .18s ease, background .18s ease}
    .liberty-panel .page-item .page-link:hover,.liberty-panel .page-item .page-link:focus-visible{transform:translateY(-2px);background:rgba(13,110,253,.18);color:#0b2c61}
    .liberty-panel .page-item.active .page-link{background:linear-gradient(135deg,#0d6efd,#6610f2);color:#fff;box-shadow:0 14px 24px rgba(13,110,253,.28)}
    .liberty-panel .page-item.disabled .page-link{opacity:.45;transform:none;box-shadow:none}
    .liberty-panel .pagination{justify-content:flex-end}
</style>
                ...
                <div class="container-fluid px-4 pt-3">
                    <?php if (isset($_GET['restore_success']) && $_GET['restore_success'] == 1): ?>
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            ...
                            $tables = $_GET['tables'] ?? '';
                            $count = $_GET['count'] ?? 0;
                            echo "Restored $count records to Status of Liberty!";
                            ?>
                            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                        </div>
                    <?php endif; ?>
                    
                    <div class="liberty-controls">
                        <div class="controls-left">
                            <input id="search-input" class="form-control" style="width:240px;" type="search" placeholder="Search name or father/mother">
                        </div>
                        <div class="controls-right">
                            <a class="btn btn-primary rounded-pill px-4" href="<?= BASE_URL ?>/liberty/insert.php">New</a>
                        </div>
                    </div>

                    <div class="row gap-3">
                        <div class="col-12">
                            <div class="card border-0 liberty-panel">
                                <div class="card-header text-white">
                                    <div class="d-flex align-items-center gap-3">
                                        <span class="header-icon" aria-hidden="true">📝</span>
                                        <div>
                                            <h5 class="card-title mb-0">Status of Liberty List</h5>
                                            <span class="card-subtitle">Central overview of liberty records</span>
                                        </div>
                                    </div>
